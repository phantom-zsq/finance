...
# -------------------common start-------------------
basic_df = pd.read_sql_query('select ts_code from stock_basic where market is not null and ts_code>=\'000001.SZ\' order by ts_code', engine)
for index, row in basic_df.iterrows():
    print(index, row[0])
    time.sleep(2)
    df = pro.suspend_d(ts_code=row[0], start_date='20210101', end_date='20211231')
