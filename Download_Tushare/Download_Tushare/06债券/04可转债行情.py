...
basic_df = pd.read_sql_query('select ts_code from manual_cb_basic order by ts_code desc', engine)
for index, row in basic_df.iterrows():
    print(index, row[0])
    try:
        df = pro.cb_daily(ts_code=row[0], start_date='19900101', end_date='19970101')
        res = df.to_sql('cb_daily_new', engine, index=False, if_exists='append', chunksize=10000)
        print(res)
        time.sleep(15)
